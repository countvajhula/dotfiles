...
function! s:remove_blank_html_lines(lines) " {{{
  while a:lines[-1] == '^\s*$'
    call remove(a:lines, -1)
  endwhile
endfunction "}}}
...
function! s:process_tag_list(line, lists) "{{{
  ...
  " change from Ping Song on forums to add html linebreaks to every line
  let g:vimwiki_ignore_newline = 0 " my addition - should probably centralize
  if !g:vimwiki_ignore_newline
    call add(lines, '<br />')
    call s:remove_blank_html_lines(lines)
  endif
